<template>
  <ion-page>
    ...
    <ion-content :fullscreen="true" color="light">
      ...
      <Questions
        v-if="type === 'questions'"
        v-for="question in PublicTestData"
        :key="question.id"
        :question="question"
        :show="true"
        @delete="HandleDelete"
      ...
      <ion-card v-if="type === 'students'" style="min-height: 480px" dir="rtl">
        ...
        <ion-card-content>
          <ion-list>
            <ion-item
              v-if="type === 'students'"
              v-for="student in PublicTestData"
              :key="student.phone"
            >
              <div class="student">
                <div class="char">
                ...
                <div class="ditails">
                  ...
                  <router-link
                    :to="`/publictest/answers/${student.phone}/${id}`"
                  >
                    <p>عرض الاجابات</p>
                  </router-link>
                  <div class="grade">
...
export default defineComponent({
  ...
  setup() {
    ...
    const PublicTestData = ref<Array<any>>([]);

    onMounted(() => {
      store.commit("isLoggedIn", router);
    });
    const getPublicTestData = () => {
      fetch(`${BaseUrl}/public_tests/techer/${type.value}/${id}`, {
        method: "GET",
        headers: {
        ...
        .then((data) => {
          ...
          PublicTestData.value = data.data;
        });
    };
